...
# 2026/01/29 21:50：モデル生存確認テストに基づく最終適合版
TOTAL_WORK_TIME = "4.5時間 + バグ取り5時間（モデル世代適合完了）"
...
st.title(f"{PROJECT_NAME} - 最新世代(2.0)適合版")

# --- サイドバー ---
api_key = st.sidebar.text_input("Google API Key", type="password")
st.sidebar.write(f"作業累計: **{TOTAL_WORK_TIME}**")
st.sidebar.info("「Googleの世代交代(1.5→2.0/2.5)に完全対応しました」")
...
if api_key and uploaded_file:
    ...
    # --- 【重要】生存確認リストにあった最新モデル名を指定 ---
    # models/gemini-2.0-flash または models/gemini-flash-latest を使用
    try:
